<template>
  <div class="container" :style="'height:'+winHeight+'px'">
      <div class="accountMainInfo">
        <div class="accountTips">抵扣券(张)</div>
        <div class="accountNum"  v-if="UserInfo!=null">{{GoldMoneystr}}</div>
      </div>
    ...
    <block v-for="(item,index) in historyList" :key="index">
       ...
       <div class="listItem">
          ...
          <div class="userNum" v-if="item.ChangeCount>0">+{{item.ChangeCount}}抵扣券</div>
          <div class="userNum" v-else>{{item.ChangeCount}}抵扣券</div>
        </div>
    </block>
  ...
  methods:{
    ...
    async GetMoneyDeatils(){
      ...
      let url="/Account/GetMoneyDeatils";
        let d ={lastTime:this.lastTime,moneytype:'gold',pageCount:50,starttime:'201801',timetype:'month',endtime:endtime};
...
this.historyList=this.historyList.concat(data.Data);
    ...
    async ViewMonthDetail(month){
      let url='/pages/monthgold/main?groupvalue='+month;
      wx.navigateTo({url});
    },
  },
